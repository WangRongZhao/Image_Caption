...
class CaptionGenerator(BaseModel):
    ...
    def build_vgg16(self):
        ...
        print('images_shape', images.get_shape())
        print('conv5_3_feats', conv5_3_feats.get_shape())
        print('reshaped_conv5_3_feats', reshaped_conv5_3_feats.get_shape())
    ...
    def build_rnn(self):
        ...
        if self.is_train:
            contexts = self.conv_feats #conv features
           ...
           # Apply the LSTM
            ...
            with tf.variable_scope("decode"):
                expanded_output = tf.concat([output,
                                             ...
                                             word_embed],
                                             axis=1)
                expanded_output1.append(expanded_output)
        ...
        if self.is_train:
            ...
            self.output = output
            self.probs = probs
        self.lstm_output = tf.concat(expanded_output1, axis=1)
        with tf.variable_scope("generate_condition", reuse=tf.AUTO_REUSE):
            condition = self.generate_condition(self.lstm_output)
    ...
    def generate_condition(self, c_var):
        conditions = \
            (self.nn.dense(c_var, self.config.GAN_EMBEDDING_DIM * 2))
        mean = conditions[:, :self.config.GAN_EMBEDDING_DIM]
        log_sigma = conditions[:, self.config.GAN_EMBEDDING_DIM:]
        return [mean, log_sigma]

    def generator(self, z_var):
        node1_0_1 = \
            (self.nn.dense(z_var, self.s16 * self.s16 * self.config.GAN_GF_DIM * 8))
        node1_0 = \
            (self.nn.batch_norm(node1_0_1))
        node1_0.reshape([-1, self.s16, self.s16, self.config.GAN_GF_DIM * 8])


        node1_1_0 = (self.nn.conv2d(node1_0, self.config.GAN_GF_DIM * 2,
                                    kernel_size=(1, 1), strides=(1, 1)))
        node1_1_1 = tf.nn.relu(self.nn.batch_norm(node1_1_0))
        node1_1_2 = (self.nn.conv2d(node1_1_1, self.config.GAN_GF_DIM * 2,
                                    kernel_size=(3, 3), strides=(1, 1)))
        node1_1_3 = tf.nn.relu(self.nn.batch_norm(node1_1_2))
        node1_1_4 = (self.nn.conv2d(node1_1_3, self.config.GAN_GF_DIM * 8,
                                    kernel_size=(3, 3), strides=(1, 1)))
        node1_1 = (self.nn.batch_norm(node1_1_4))

        node1 = tf.nn.relu(tf.add(node1_1, node1_0))


        node2_0_0 = \
            (node1.apply(tf.image.resize_nearest_neighbor, [self.s8, self.s8]))
        node2_0_1 = \
            (self.nn.conv2d(node2_0_0, self.config.GAN_GF_DIM * 4,
                            kernel_size=(3, 3), strides=(1, 1)))
        node2_0 = \
            (self.nn.batch_norm(node2_0_1))

        node2_1_0 = \
            self.nn.batch_norm(self.nn.conv2d(node2_0, self.config.GAN_GF_DIM * 1,
                                              kernel_size=(1, 1), strides=(1, 1))).apply(tf.nn.relu)
        node2_1_1 = \
            self.nn.batch_norm(self.nn.conv2d(node2_1_0, self.config.GAN_GF_DIM * 1,
                                              kernel_size=(3, 3), strides=(1, 1))).apply(tf.nn.relu)
        node2_1 = \
            self.nn.batch_norm(self.nn.conv2d(node2_1_1, self.config.GAN_GF_DIM * 4,
                                              kernel_size=(3, 3), strides=(1, 1))).apply(tf.nn.relu)
        #
        node2 = \
            (node2_0.
        ...
        node3_0 = \
            tf.image.resize_nearest_neighbor(node2, [self.s4, self.s4])

        node3_1 = \
            tf.nn.relu(self.nn.batch_norm(self.nn.conv2d(node3_0, self.config.GAN_GF_DIM * 2,
                                              kernel_size=(3, 3), strides=(1, 1))))
        node3_2 = \
            tf.image.resize_nearest_neighbor(node3_1, [self.s2, self.s2])

        node3_3 = \
            tf.nn.relu(self.nn.batch_norm(self.nn.conv2d(node3_2, self.config.GAN_GF_DIM,
                                              kernel_size=(3, 3), strides=(1, 1))))
        node3_4 = \
            tf.image.resize_nearest_neighbor(node3_3, [self.s, self.s])

        output_tensor = \
            tf.nn.relu(self.nn.conv2d(node3_4, 3, kernel_size=(3, 3), strides=(1, 1)))

        return output_tensor

    # d-net
    def context_embedding(self, lstm_output):
        template = \
            leaky_rectify(self.nn.dense(lstm_output, 128), leakiness=0.2)
        return template
